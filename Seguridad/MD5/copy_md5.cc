...
vector< bitset<8> > to_bytes_array(string st);
void append_padding_bits(vector< bitset<8> >* bytes_vec);
void append_length(vector < bitset<8> >* bytes_vec, int original_size);

vector< bitset<32> > init_ABCDbuffer();
vector < bitset<32> > init_CosTable();

bitset<32> F(bitset<32> X,bitset<32> Y, bitset<32> Z);
bitset<32> G(bitset<32> X,bitset<32> Y, bitset<32> Z);
bitset<32> H(bitset<32> X,bitset<32> Y, bitset<32> Z);
bitset<32> I(bitset<32> X,bitset<32> Y, bitset<32> Z);

void CLS(bitset<32> &bits, int t);
bitset<32> fixBigEndian(bitset<32> current_32bit);
void FF(bitset<32> &a, bitset<32> &b, bitset<32> &c, bitset<32> &d,
		int k, int s, int i, vector < bitset<32> > X);
void GG(bitset<32> &a, bitset<32> &b, bitset<32> &c, bitset<32> &d,
		int k, int s, int i, vector < bitset<32> > X);
void HH(bitset<32> &a, bitset<32> &b, bitset<32> &c, bitset<32> &d,
		int k, int s, int i, vector < bitset<32> > X);
void II(bitset<32> &a, bitset<32> &b, bitset<32> &c, bitset<32> &d,
		int k, int s, int i, vector < bitset<32> > X);
		
bitset<512>* getNextChunk(vector< bitset<8> > msg, int i);
vector< bitset<32> > breakChunk(bitset<512> b);
string getHash(vector< bitset<32> > ABCD);
string MD5(string st);

/* Variables globlales */
vector < bitset<32> > T = init_CosTable();
...
*/
	cout << "Hash de \"" << test_1 << "\": " << MD5(test_1) << endl;
	/*
	cout << "Hash de \"" << test_2 << "\": " << MD5(test_2) << endl;
	cout << "Hash de \"" << test_3 << "\": " << MD5(test_3) << endl;
	cout << "Hash de \"" << test_4 << "\": " << MD5(test_4) << endl;
	cout << "Hash de \"" << test_5 << "\": " << MD5(test_5) << endl;
	cout << "Hash de \"" << test_6 << "\": " << MD5(test_6) << endl;
	*/
}
// ======================== MAIN =====================================
